#include "../double/double_scalbn/double_scalbn.h"
#include "../double/double_fabs/double_fabs.h"
...
int __kernel_rem_pio2(double *x, double *y, int e0, int nx, int prec, const __int32_t *ipio2) {
 ...
 j = jv-jx; m = jx+jk;
 for(i=0;i<=m;i++,j++) f[i] = (j<0)? zero_krempio : (double) ipio2[j];
...
recompute:
 ...
 if(ih>0) {
  ...
  if(carry==0) {
   ...
   carry = 1; iq[i] = 0x1000000- j;
     ...
     if(ih==2) {
  z = one_krempio - z;
  if(carry!=0) z -= scalbn_double(one_krempio,(int)q0);
     }
 }
 ...
 for(i=jz;i>=0;i--) {
     q[i] = fw*(double)iq[i]; fw*=twon24_krempio;
 }
...
double __kernel_tan(double x, double y, int iy){
 ...
 if(ix<0x3e300000)
     {if((int)x==0) {
         __uint32_t low;
  do { ieee_double_shape_type gl_u; gl_u.value = (x); (low) = gl_u.parts.lsw; } while (0);
  if(((ix|low)|(iy+1))==0) return one_ktan/fabs_double(x);
